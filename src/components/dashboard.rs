...
//~ }
    ...
    pub fn check_all_now(&self) {
        // TODO: Implement the logic for the "Check All Now" action.
        for project_task in self.project_tasks.iter() {
            // Perform the check for the current project_task
            // This is a placeholder, you'll need to call the actual function to perform the check
            self.perform_check(project_task);
        }
    }

    pub fn timer_mode(&self) {
        // TODO: Implement the logic for the "Timer Mode" action.
        let duration = self.timer_settings_duration_hours * 3600.0;
        let checks = self.timer_settings_checks;

        for _ in 0..checks {
            self.check_all_now();
            let sleep_duration = duration / checks as f64;
